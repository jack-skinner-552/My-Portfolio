# Program asks for a Pokemon Type and displays the pokemon who are resistant aganst given type moves.
...
if response.status_code == 200:
    ...
    type_strength_names = [strength["name"] for strength in type_strengths]
    ...
    # Limit API call to 905 because Pokemon ID information for Generation 9 is incorrect
    all_pokemon_url = "https://pokeapi.co/api/v2/pokemon?limit=905"
    ...
    if response.status_code == 200:
        ...
        for pokemon in tqdm(all_pokemon_list):
            ...
            if (set(type_weakness_names) & set(pokemon_types)) or (set(type_no_damage_names) & set(pokemon_types)):
                if not set(type_strength_names) & set(pokemon_types):
                    type_strong_pokemon.append((pokemon_id, pokemon["name"], pokemon_types))

        # Sort the list of weak Pokemon by ID number
        type_strong_pokemon.sort()

        # Print the names of the weak Pokemon, their ID numbers, and their types
        print(f"Pokemon strong to {move_type} moves:")
        for pokemon in type_strong_pokemon:
